...
int writetolog(std::string completeDom, std::string domain, std::string client_ip, std::string domain_ip, std::string response, std::string zoneName);
...
int QueryMainNameserver(ST Listst, SSPM SiSoPoMain, struct sockaddr_in serverInfo, std::string* resp, std::vector<int> statusParser, int* dom, std::string doma, int *btcnt, std::vector<std::string> &ListUniResp);
...
int QueryDatabases(std::string bl, std::string completeDomain, std::vector <int> statusParser, int main_s, socklen_t socket_l, struct sockaddr_in server_i, std::vector<std::string> &ListUniResp){
	...
	for(int j = 0; j < (int)NewURLs.size(); j++){
		...
		if(malUni.size() != 0){
			for(int i = 0; i < (int)malUni.size(); i++ ){
				...
				// Original domain will be logged by normal query
				// So no need to log again
				if(j == 0) ListUniResp.push_back(malUni[i]);
				else
				{
	...
	for(int j = 0; j < (int)NewURLs.size(); j++){
		...
		if( chiUni.size() != 0 ){
			for(int i = 0; i < (int)chiUni.size(); i++){
				...
				// Original domain will be logged by normal query
				// So no need to log again
				if(j == 0) ListUniResp.push_back(chiUni[i]);
				else
				{
...
int writetolog( std::string compleDom,std::string domain, std::string client_ip, std::string domain_ip, std::string response, std::string zoneName)   
{
	int res;
	res = open("Blocked_Domains", O_WRONLY | O_NONBLOCK);

	if(res < 0 && (errno != EEXIST)) 
	{
		std::cout << "WriteToLog: Error reading the named pipe" << std::endl;
		return 1;
	}
...
struct sockaddr_in server_info,  
	...
	std::vector <int> statusParser){
		...
		std::vector<std::string> ListUniResp;
		domainFound = QueryDatabases(" ",domain, statusParser, main_socket, socket_length, server_info, ListUniResp); 
#ifdef DEBUG
		...
		if(domainFound == 0 
			...
			printf("DB lookup for domain '%s' yields code :%d. \n", domain.c_str(), domainFound); 

#ifdef DEBUG
			std::cout << "IPadd: " << IPadd << std::endl;
#endif						
			thMain.add_thread(new boost::thread(QueryMainNameserver, listST, SiSoPo, BackMainServer, &resp, statusParser, &domainFound, domain, &btcnt, ListUniResp));
		}
...
//int NS;
int QueryMainNameserver( ST Listst, SSPM SiSoPoMain, struct sockaddr_in server_info, std::string* resp, std::vector<int> status, int* dom, std::string doma, int *btcnt, std::vector<std::string> &ListUniResp){
	...
	mDNSstructure->QuickTestCNIP(request_size, response_buf, bytecount, IPlists, &RealHostname);
	delete mDNSstructure;	

	for(int i =0 ;i < (int)IPlists.size(); i++){
			for(int j=0 ;j < (int)ListUniResp.size(); j++) {
				if (*dom == 3)
					writetolog(doma, doma, "Disabled", IPlists[i], ListUniResp[j], "");
		}
	}
	...
	if(*dom != 3 && *dom != 2 ){
		...
		if(RealHostname.compare(" "))
			*dom = QueryDatabases(doma, RealHostname, status, SiSoPoMain.MSocket, SiSoPoMain.Socketdes, SiSoPoMain.servInfo, uni); 
		...
		for(int i =0 ;i < (int)IPlists.size(); i++){
			...
			if( ipUni.size() != 0){
				// DCO : Possible Bug
				// Was using variable i here again
				// changed to j => Does C++ allow this ?
				for(int j = 0; j < (int)ipUni.size(); j++) {
					if(!doma.empty()) {
						writetolog(doma, doma, "Disabled", IPlists[i], ipUni[j], "");	
					}
				}
				found = 1;
			}//end if statement
