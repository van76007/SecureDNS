...
int write2lg(std::string completeDom, std::string domain, std::string client_ip, std::string domain_ip, std::string response, std::string zoneName);
	...
	for(int i=0;i<ntohs(dns->ans_count);i++)
	{
		...
		else
		{
			...
			for(int h = 0; h < int(strlen(ansData.c_str())); h++ )
				answers[i].rdata[h] = TempAnsData[h];
...
int write2lg(std::string compleDom, std::string domain, std::string client_ip, std::string domain_ip, std::string response, std::string zoneName)
{
	int res;
	...
	{
		std::cout << "Write2Lg: Error reading the named pipe" << std::endl;
		return 1;
	}
