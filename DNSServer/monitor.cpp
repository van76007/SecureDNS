...
void buildIPCDB(std::string IPDB){
    ...
    std::string completeQueryIPlist = "SELECT distinct blocked_ips.ip, categories.unifiedresponse FROM blocked_ips inner join categories on blocked_ips.category=categories.name";
...
int main(int argc, char* argv[]) {
	...
	std::cout << "TEST DCO " << (bool)(false && false) << "\n";
	std::cout << "TEST DCO " << (bool)(0 && 0) << "\n";

	if(  !(boost::filesystem::exists( malwarecdb1 ) && boost::filesystem::exists(malwarecdb2)) ) {

		std::cout << "Malware1 exists : " << !boost::filesystem::exists(malwarecdb1) << std ::endl;
		std::cout << "Malware2 exists : " << boost::filesystem::exists(malwarecdb2) << std ::endl;

  		malwarecdb = malwarecdb1;
		if ( boost::filesystem::exists( malwarecdb1 ) ) {malwarecdb = malwarecdb2;}
		std::cout << "building Malware" << std::endl;
		std::cout << "Malware cdb is "<< malwarecdb << "\n"; 
