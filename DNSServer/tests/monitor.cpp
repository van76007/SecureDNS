...
void buildMalCDB(std::string malDB){
	...
	if( boost::filesystem::exists( malDB ) ){
     ...
     conn = mysql_init(conn);
     if( NULL == conn){
	std::cout << "Connection to Null"  << std::endl;
	mysql_close(conn);
   	mysql_library_end();
  	 return;
     }
	std::cout << "Before createconnection" << std::endl;
	conn = createconnection();
...
int main(int argc, char* argv[]) {
	...
	std::string malwarecdb1 = "malware1.cdb"; std::string malwarecdb2 = "malware2.cdb"; std::string malwarecdb;
	std::string whitecdb1 = "whitelist1.cdb"; std::string whitecdb2 = "whitelist2.cdb"; std::string whitecdb;
	std::string childcdb1 = "child1.cdb"; std::string childcdb2 = "child2.cdb";std::string childcdb;
	std::string IPcdb1 = "IP1.cdb"; std::string IPcdb2 = "IP2.cdb";std::string IPcdb;
