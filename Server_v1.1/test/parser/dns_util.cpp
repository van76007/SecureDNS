...
// Given Name, get vector of domain/subdmains from it's question
vector<string> getSubsFromName(string Name, string& domain) {
    vector<string> rs; // The last element will be domain
	domain = "";
    vector<string> strs;
	...
	int offset = 0;
	// Intermediate result: subdomain/domain. Final one will be domain 
...
// Given DNS_packet, get last CNAME from it's answer
string getCName(struct DNS_Packet* p) {
    string rs = "";
    if (p->ancount) {
        // Multiple answers but the last one is A record
		if (p->answer[p->ancount - 1]->dnsType==TYPE_A ) {
			std::string cname(p->answer[p->ancount - 1]->name, strlen(p->answer[p->ancount - 1]->name));
			rs = cname; 
		}
    }
    return rs;
...
string getID(struct DNS_Packet* p) {
    // unsigned short p->id;
    char buff[14];
    return string(buff, sprintf(buff, "%d", p->id));
}
...
vector<string> getCNames(struct DNS_Packet* p) {
	...
	if (p->ancount) {
	    for (int i = 0; i < p->ancount; i++) {
			if (p->answer[i]->dnsType==TYPE_CNAME ) {
				std::string cname(p->answer[p->ancount - 1]->name, strlen(p->answer[p->ancount - 1]->name));
				std::string tmp_cname = cname;
				rs.push_back(tmp_cname);
			} // end if
			
			// Add last CNAME that would be resolved to IPs
