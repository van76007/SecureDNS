...
namespace screenDNS {
		...
		for ( it=strs.begin() ; it < strs.end()-1; it++ ) {
			...
			if ( offset < Name.size() ) {
				...
				if ( it < strs.end()-2 ) {
					...
					#ifdef DBUG
						cout << "Push back sub-domain:_" << domain << "_\n";
					#endif
				} else if ( it == strs.end()-2 ){
					domain = "." + domain;
					rs.push_back(domain); // Push back sub-domain
					#ifdef DBUG
						cout << "Push back TLD:_" << domain << "_\n";
					#endif
				}
			}
		} // end for
	...
	bool forwarder::verify_vector_InWhite(vector<string>& input) 
	{
		...
		for(it = input.end()-1; it >= input.begin(); it--) {
			#ifdef DBUG
				cout << "verifying domain in White:_" << (*it) << "_\n";
			#endif
			if (m_reader_WHITELIST->lookup(*it)) {
				rs = true;
				break;
			}
		}
							...
							) 
	{
		...
		if ( tag == atoi(Config::Instance()->get_port_protection_plus_1().c_str()) )
		{
			...
			bool rs_CHILDPORN = m_reader_CHILDPORN->lookupUnifiedResponse(domain,
													uni_resp_CHILDPORN); 
			...
			bool rs_MALWARE = m_reader_MALWARE->lookupUnifiedResponse(domain,
												uni_resp_MALWARE);                
		...
		if ( tag == atoi(Config::Instance()->get_port_protection_plus_3().c_str()) )
		{
			...
			vector<string> uni_resp_CHILDPORN;
			rs = m_reader_CHILDPORN->lookupUnifiedResponse(domain,
														   uni_resp_CHILDPORN);
	...
	void forwarder::doLoggingReason(string blocked_domain, string FQDN, vector<string> uni_resp)
	{
		if (uni_resp.size() > 0) {
	...
	void forwarder::doLoggingStat(string blocked_domain, string FQDN, vector<string> IPs, vector<string> uni_resp)
	{	
								...
								udp::endpoint remote_endpoint) 
    {
        ...
        if (blockedReq) {
			...
			// Log reason
			doLoggingReason(blocked_domain, FQDN, uni_resp);
        }
		...
		if (recv_len > 0) {
			...
			if (!blockedResp) {
				...
				if (CNAMEInWhiteList)
					doLoggingForWhiteList(FQDN, "230", "cname whitelist", "screenDNS");
				
				// Log for statistics purpose
				if (IPInBlackList) {
					vector<string> blockedIP;
					blockedIP.push_back(response_blocked_domain);
					doLoggingStat(response_blocked_domain, FQDN, blockedIP, response_uni_resp);
				} else {
					doLoggingStat(response_blocked_domain, FQDN, IPs, response_uni_resp);
